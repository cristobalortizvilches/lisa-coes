---
title: "Descriptivos CAIS Pre-ALAS"
...
cais_v2 <- lisa_v2_75 %>% 
  ...
  dplyr::select(-c(created, modified, ended, expired, start_01)) %>% # excluyo variables sobre info de sesión
...
legend = "Fuente CAIS (2024), N = 70."

cais_test_annos <- cais_v2 %>% 
  select(session, mod01_1, mod08_1)
...
```{r bca-sexo, echo=FALSE, warning=FALSE}
...
```{r prac1, eval=FALSE, include=FALSE}
cais_v2 %>%
  select(mod02_3) %>% 
  drop_na() %>% 
